 ...
 *
 * It works just like a singly linked list but allows to traverse the list
 * backwards, from tail to head, allowing for easier access to elements
 * at the end of the list.
 *
 * A doubly linked lists might perform slightly better than a singly linked
 * list when accessing pushing/removing from the end of the list or for
 * accessing elements closer to the end of the list, since we are able
 * to access them from the tail and traverse it backwards. However, they
 * require more memory to keep all the references.
 *
 *      [list] ---> [node1] <---> [node2] <---> [...nodeN] <--- [list]
 *
 * > **ATTENTION:** pay close attention to the arrows on the diagram above
 *   as it is certainly not the best format to represent a linked list.
 *   Note that **none** of the nodes keep references to the list itself and
 *   that `list` represents the **same** doubly linked list instance.
 *
 * > **NOTE:** I couldn't find any convincing use case for a doubly
 *   linked lists. Unfortunately I've searched for one only after I had
