...
describe('DoublyLinkedList', function () {
  it('is accessible', function () {
    expect(DoublyLinkedList).to.be.defined;
  });
  ...
  it('is a constructor', function () {
    ...
    expect(list).to.be.instanceof(DoublyLinkedList);
  });
  ...
  describe('instance', function () {
    ...
    describe('.toString()', function () {
      ...
      it('uses the same representation as arrays', function () {
        ...
        var emptyList = new DoublyLinkedList();
    ...
    describe('.clone()', function () {
      ...
      beforeEach(function () {
        list.unshift('foo', Date.now(), {});
        clone = list.clone();
      });

      it('is a function', function () {
        expect(list.clone).to.be.a('function');
      });

      it('returns a DoublyLinkedList', function () {
        expect(clone).to.be.instanceof(DoublyLinkedList);
      });

      it('has its own nodes', function () {
        expect(list._get(0)).to.not.equal(clone._get(0));
        expect(list._get(1)).to.not.equal(clone._get(1));
        expect(list._get(2)).to.not.equal(clone._get(2));
      });

      it('contains the same values', function () {
        expect(clone.length).to.equal(list.length);

        expect(clone.get(0)).to.equal(list.get(0));
        expect(clone.get(1)).to.equal(list.get(1));
        expect(clone.get(2)).to.equal(list.get(2));
      });
    });
  });
});